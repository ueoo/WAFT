...
class InferenceWrapper(object):
    ...
    def inference_padding(self, image):
        ...
        pad_w = max(w - W, 0)
        padded_image = F.pad(
            image, (pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2), mode="constant", value=0
        )
        ...
        for h_ij, w_ij in patches:
            ...
            if output is None:
                ...
                for key in output_ij.keys():
                    if "flow" in key:
                        output[key] = [
                            torch.zeros((n, 2, h, w), device=image1.device) for _ in range(len(output_ij[key]))
                        ]
                    elif "info" in key:
                        output[key] = [
                            torch.zeros((n, 4, h, w), device=image1.device) for _ in range(len(output_ij[key]))
                        ]
                    else:
                        output[key] = [
                            torch.zeros((n, 2, h, w), device=image1.device) for _ in range(len(output_ij[key]))
        ...
        for i in range(len(output["flow"])):
            for key in output.keys():
                output[key][i] = output[key][i][
                    :, :, inf_pad_h // 2 : inf_pad_h // 2 + H, inf_pad_w // 2 : inf_pad_w // 2 + W
                ]
            ...
            for key in output.keys():
                if "flow" in key:
                    output[key][i] = F.interpolate(
                        output[key][i], scale_factor=0.5**self.scale, mode="bilinear", align_corners=True
                    ) * (0.5**self.scale)
                else:
                    output[key][i] = F.interpolate(
                        output[key][i], scale_factor=0.5**self.scale, mode="bilinear", align_corners=True
