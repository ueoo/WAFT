...
class TwinsFeatureEncoder(nn.Module):
    def __init__(self, frozen=True):
        super().__init__()
        self.backbone = timm.create_model("twins_svt_large", pretrained=True, features_only=True)
        if frozen:
        ...
        self.refine = nn.ModuleList(
            [_make_fusion_block(self.features, False, size=None) for _ in range(len(self.out_channels))]
        )
        self.final = nn.ConvTranspose2d(
            in_channels=self.features, out_channels=self.features // 2, kernel_size=4, stride=2, padding=1, bias=True
        )
