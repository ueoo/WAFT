...
if __name__ == "__main__":
    ...
    H, W = image1.shape[:2]
    new_coords = flow + np.stack(np.meshgrid(np.arange(W), np.arange(H), indexing="xy"), axis=-1)
    warped_image1 = cv2.remap(image2, new_coords.astype(np.float32), None, interpolation=cv2.INTER_LINEAR)
